/* ═══════════════════════════════════════════════════════════════
   건축물 우수관경 산정 시스템 — Service Worker
   KDS 31 30 35 : 2021 | ARCHITECT KIM MANMIN · Ver-2.0
   전략: Cache-First (로컬 자산) + Network-First (외부 CDN)
   ═══════════════════════════════════════════════════════════════ */

const CACHE_NAME   = 'rain-pipe-v2.0';
const CDN_CACHE    = 'rain-pipe-cdn-v2.0';
...
self.addEventListener('push', event => {
  const data = event.data?.json() ?? { title: '우수관경 산정 시스템', body: '업데이트가 있습니다.' };
